...
public class bplustree implements Serializable{
    ...
    public void saveIndex(String strTableName, String strIndexName) throws DBAppException {
        try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream("tables/" + strTableName + "/" + strIndexName + ".bin"))) {
            oos.writeObject(this);
        } catch (IOException e) {
            throw new DBAppException("can't save Index");
        }
    }

    public static bplustree getIndex(String strTableName, String strIndexName) throws DBAppException {
        String fileName = "tables/" + strTableName + "/" + strIndexName + ".bin";
     ...
     * @param targetKey: target key value of dictionary pair being searched for
     * @return index of the target value if found, else a negative value
     */
    private int binarySearch(DictionaryPair[] dps, int numPairs, Object targetKey) {
        Comparator<DictionaryPair> c = new Comparator<DictionaryPair>() {
            ...
            public int compare(DictionaryPair o1, DictionaryPair o2) {
                ...
                if (o1.key instanceof String && o2.key instanceof String) {
                    ...
                    // Throw an exception or handle incompatible key types here
                    throw new IllegalArgumentException("Unsupported key type in DictionaryPair");
                }
            }
        };
        return Arrays.binarySearch(dps, 0, numPairs, new DictionaryPair(targetKey,new Vector<>() ), c);
    }
     ...
     * @paramkey: the unique key that lies within the dictionary of a LeafNode object
     * @return the LeafNode object that contains the key within its dictionary
     */

    private int compareKeys(Object key1, Object key2) {
        // Implement comparison logic based on your key types:
        if (key1 instanceof String && key2 instanceof String) {
            ...
            // Handle incompatible key types (throw an exception or return a specific error code)
    ...
    private LeafNode findLeafNode(Object key) {
        ...
        for (i = 0; i < this.root.degree - 1; i++) {
            // Use the compareKeys method for flexible key comparisons
    ...
    private LeafNode findLeafNode(InternalNode node, Object key) {
        ...
        // Initialize keys and index variable (assuming keys are type K)
        Object[] keys = node.keys;
        int i;

        // Find next node on path to appropriate leaf node
        for (i = 0; i < node.degree - 1; i++) {
            // Use a helper method for comparing keys of different types
            if (compareKeys(key, keys[i]) < 0) {
                break;
            }
        }
    ...
    /*~~~~~~~~~~~~~~~~ API: DELETE, INSERT, SEARCH, UPDATE VALUE ~~~~~~~~~~~~~~~~*/
    ...
    public void delete(Object key, String value) {
        if (isEmpty()) {
            ...
            if (dpIndex < 0) {
                ...
                if(ln.dictionary[dpIndex].value.remove(value) && ln.dictionary[dpIndex].value.isEmpty()) {
                    ln.delete(dpIndex);
                    ...
                    if (ln.isDeficient()) {
						...
						/* Insert borrowed dictionary pair, sort dictionary,
						   and delete dictionary pair from sibling */
                            ...
                            if (compareKeys(borrowedDP.key, parent.keys[pointerIndex - 1]) < 0) {
						...
						/* Insert borrowed dictionary pair, sort dictionary,
					       and delete dictionary pair from sibling */
                            ...
                            // Update key in parent if necessary
                            int pointerIndex = findIndexOfPointer(parent.childPointers, ln);
                            if (compareKeys(borrowedDP.key, parent.keys[pointerIndex]) >= 0) {
    ...
    public void insert(Object key, String value) {
        if (isEmpty()) {
            // Create leaf node as the first node in the B+ tree
            LeafNode ln = new LeafNode(this.m, new DictionaryPair(key, new Vector<>(Arrays.asList(value))));
            // Set it as the first leaf node
            this.firstLeaf = ln;
        } else {
            // Find leaf node to insert into
            LeafNode ln = (this.root == null) ? this.firstLeaf : findLeafNode(key);
            // Search for the key within the leaf node's dictionary
            DictionaryPair[] dictionary = ln.dictionary;
            int index = binarySearch(dictionary, ln.numPairs, key);
            // If index is negative, the key doesn't exist in the B+ tree
            if (index < 0) {
                // Key not found, insert a new key-value pair
                if (!ln.insert(new DictionaryPair(key, new Vector<>(Arrays.asList(value))))) {
                    // Leaf node is overfull, split it
                    ln.dictionary[ln.numPairs] = new DictionaryPair(key, new Vector<>(Arrays.asList(value)));
                    ln.numPairs++;
                    sortDictionary(ln.dictionary);
                    int midpoint = getMidpoint();
                ...
                values.add(value); // Allow duplicates
            }
        }
    }
    ...
    public void updateValue(Object key, String oldValue, String newValue) {
        ...
        // If index is negative, the key doesn't exist in the B+ tree
        if (index < 0) {
            System.err.println("Invalid Update: Key not found.");
            return;
        }

        // Check if the vector associated with the key contains the old value
        if (!dictionary[index].value.contains(oldValue)) {
            System.err.println("Invalid Update: Old value not found.");
            return;
        }

        // Update the value
        dictionary[index].value.set(dictionary[index].value.indexOf(oldValue), newValue);
    }
    ...
    public ArrayList<String> search(Object lowerBound, Object upperBound) {
        ...
        while (currNode != null) {
            ...
            for (DictionaryPair dp : dps) {
                ...
                if (compareKeys(lowerBound, dp.key) <= 0 && compareKeys(dp.key, upperBound) <= 0) {
     ...
     */
    public bplustree(int m, String strColName, String indexName) {
        this.m = m;
        this.root = null;
    ...
    public class DictionaryPair implements Comparable<DictionaryPair>, Serializable {
         ...
         */
        public DictionaryPair(Object key, Vector<String> value) {
            this.key = key;
            this.value = value;
        }
        ...
        try {
            ...
            while (sc.hasNextLine()) {
                ...
                switch (tokens[0]) {
                    ...
                    case "Initialize":
                        bpt = new bplustree(Integer.parseInt(tokens[1]),"", "index1");
                        break;
                    ...
                    case "Delete":
                        bpt.delete(Integer.parseInt(tokens[1]), tokens[2]);
                        break;
                    ...
                    case "Search":
                        ...
                        // Perform search operation
                            ...
                            Vector<String> value = bpt.search(Integer.parseInt(tokens[1]));
